...
in the bootstrap response, and the chosen initial target context, and redeem
the bootstrap context at the token endpoint using at least:
...
then creates the initial `actc` and issues a profiled access token containing at
least `iss`, `actp`, `acti`, `sub`, `jti`, `aud`, `exp`, and `actc`, plus
`act` as required by the selected disclosure mode.
...
* sign `actor_chain_step_proof` over that actor-visible chain, the preserved
  workflow `sub` string for the hop, and the next target context; and
...
proof for the active profile. It MUST verify that the step proof `ctx` equals
the active profile's domain-separation string, that the step proof `sub` equals
the preserved workflow `sub` string for the hop being extended, and that the
step proof's `act` equals the profile-defined actor-visible chain for the hop.
It MUST verify append-only processing of that actor-visible chain, update
`actc`, and return a profiled access token whose disclosed `act`
representation matches the selected disclosure mode. Replay, idempotency, and multiple-successor
handling for submitted step proofs are defined in {{replay-freshness}}.
...
actor identity, disclosed actor chain state visible to the current actor,
...
target context, or recipient is not permitted or not supported, or a
Refresh-Exchange attempts to retarget to a different recipient.
...
* preserve-state authorization policy failure; and
* submitted prior state inconsistent with the claimed profile state.
...
the returned profiled access token `act` MAY disclose any ordered subsequence of that
...
Disclosure profile, the returned profiled access token `act` MUST contain only the
outermost current actor. The `target_context` member MUST conform to the
representation defined in {{target-context}}.
...
recipient MUST preserve the relevant disclosed actor chain state unchanged. For `sub`,
...
**Part III. Informative Rationale, Examples, and Operational Guidance**
...
Those profiles therefore provide non-repudiation over the signed disclosed actor chain
...
| --- | --- | --- |
| Stable generation and preservation of `acti`, using a CSPRNG with at least 122 bits of entropy (for example, standard UUIDv4 or stronger generation) | {{actor-chain-identifier}} | [ ] |
...
| Preserve-state handling for cross-domain re-issuance and Refresh-Exchange, if supported | {{cross-domain-reissuance}}; {{refresh-exchange}} | [ ] |
